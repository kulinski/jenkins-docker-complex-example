// Borrowed from https://gist.github.com/jglick/0aa389c053196e38e2a1
node {
    // https://registry.hub.docker.com/_/maven/
    def mavenDocker = docker.image('maven:3.3.9-jdk-8-onbuild');
    
    stage 'Mirror'
    // First make sure the slave has this image.
    // (If you could set your registry below to mirror Docker Hub,
    // this would be unnecessary as mavenDocker.inside would pull the image.)
    mavenDocker.pull()
    ...
    //docker.withRegistry('https://docker.example.com/', 'docker-registry-login') {
        ...
        // use the spring-petclinic Dockerfile (see above 'mavenDocker.inside()' block)
        // to build container that can run the app. The Dockerfile is in the cwd of the active workspace
        // (see above mavenDocker.inside() block), so we pass '.' as the build PATH param. The Dockerfile
